 ...
 * @param {string} tipo - Tipo da lista (renovacao, novo, entrada, saida)
 */
...
function atualizarGrafico(inicial, renovacoes, novos, entradas, saidas, final) {
  ...
  const calcularPorcentagem = (valor) => {
    if (valor <= 0) return 0;
    return Math.max((valor / maxValor) * 100, 2);
  };

  // Atualiza larguras das barras com animação
  setTimeout(() => {
...
function exportarExcel() {
  ...
  const resumoData = [
    ...
    ['SALDO RENOVAÇÕES', totRenov],
    ['SALDO NOVOS CLIENTES', totNovos],
    ['SALDO ENTRADAS DIVERSAS', totEntradas],
    ['SALDO SAÍDAS DIVERSAS', totSaidas],
    ['SALDO FINAL EM CONTA', final],
    ['', ''],
    ['DIFERENÇA', final - (inicial + totRenov + totNovos + totEntradas - totSaidas)],
    ['STATUS', Math.abs(final - (inicial + totRenov + totNovos + totEntradas - totSaidas)) < 0.01 ? 'CONCILIADO' : 'NÃO CONCILIADO']
  ];
...
/**
 * Carrega dados no sistema
 * @param {Object} data - Objeto com dados a carregar
 */
